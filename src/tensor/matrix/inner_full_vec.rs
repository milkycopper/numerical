...
impl<T: CoreFloat> MatrixBaseOps<T> for Matrix<T, FullVec<T>> {
    fn add(&self, rhs: &Self) -> Self {
        let mut inner = vec![T::default(); self.shape.area_size()];
    ...
    fn mul(&self, rhs: &Self) -> Self {
        ...
        let mut mat = Self::new_with_vec(shape, vec![T::default(); shape.area_size()]);
        for row in 0..self.row_size() {
            for col in 0..rhs.col_size() {
                for i in 0..self.col_size() {
                    mat[(row, col).into()] += self[(row, i).into()] * rhs[(i, col).into()];
                }
            }
        }
